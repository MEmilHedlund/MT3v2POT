...
@torch.no_grad()
def output_truth_plot(trajectories, ax, prediction, labels, matched_idx, batch, params, training_example_to_plot):
    ...
    for i in range(len(out)):
        if i in indices[0]:
            ...
            #for key, value in trajectories[training_example_to_plot].items():
            #random_color = (random.random(), random.random(), random.random())

            traj_values = trajectories[training_example_to_plot][truth_idx]
            ax.scatter(traj_values[:,0], traj_values[:,1], marker='D', color="k", s=2)
            ...
            # Plot missed? predictions (is this false predictions?)
