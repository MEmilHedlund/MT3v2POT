...
if __name__ == '__main__':
   #os.environ["PYDEVC_WARN_EVALUATION_TIMEOUT"] = 10000
    ...
    print('Training Devices:',params.training.device, '')
    ...
    data_generator = DataGenerator(params) # Generates objects, their measurements and labels etc.

    # Create losses for training and evaluation (evaluation = validation??)
    mot_loss = MotLoss(params)
    mot_loss.to(params.training.device) #VARFÖR 2??
    contrastive_loss = ContrastiveLoss(params)
    mot_loss_eval = MotLoss(eval_params)
    mot_loss_eval.to(eval_params.training.device) #VARFÖR 2??
    ...
    if params.debug.enable_plot or params.debug.save_plot_figs:
        ...
        loss_ax.grid('on')
        loss_line, = loss_ax.plot([1], label='Loss', c='C0')
    ...
    if True: #VAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA? vadå if true!?!
        losses = []
        last_layer_losses = []
        c_losses = []
        f_losses = []
    ...
    for i_gradient_step in range(params.training.n_gradient_steps): #Här är själva träningen, börja här och gå igenom steg för steg
        logs = {}
        for i_split_step in range(n_splits):
            try:
                batch, labels, unique_ids, unique_label_ids, trajectories = data_generator.get_batch() #Hämtar datan
                
                prediction, intermediate_predictions, encoder_prediction, aux_classifications, _ = \
                    model.forward(batch) #Nätverket tänker
                ...
                # Compute quantiles for matched and unmatched predictions #FATTAR INTE VAD HAN SYSSLAR MED HÄR VAD ÄR EN QUANT????
        ...
        if (params.debug.enable_plot and i_gradient_step % params.debug.plot_interval == 0) or \
            ...
            if params.debug.print_reduce_lr_messages:
                # Assuming you're using a single parameter group for simplicity
                current_lr = scheduler.optimizer.param_groups[0]['lr']
                print(f"Current learning rate: {current_lr}")
                # Or for all parameter groups:
                current_lrs = [param_group['lr'] for param_group in scheduler.optimizer.param_groups]
                print(f"Current learning rates: {current_lrs}")

            if percent_ax.collections:  # This checks if collections is not empty
                for artist in percent_ax.collections:
                    artist.remove()
            ...
            output_ax.cla()
            output_ax.grid('on')
            output_truth_plot(trajectories , output_ax, prediction, labels, indices, batch, params, training_example_to_plot=0)
            output_ax.set_xlim([-params.data_generation.field_of_view.max_range*0.05, params.data_generation.field_of_view.max_range*1.05])
